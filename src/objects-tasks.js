...
function mergeObjects(objects) {
  const res = {};
  const entries = objects.flatMap((el) => Object.entries(el));
...
function sellTickets(queue) {
  ...
  queue.forEach((pay) => {
    if (pay === ticketCost) {
      kassa += pay;
    } else if (pay > ticketCost) {
      if (kassa < pay - ticketCost) {
        boo = false;
      } else {
        boo = true;
      }
...
function group(array, keySelector, valueSelector) {
  ...
  countries.forEach((el) => {
    const elObj = array.filter((item) => Object.values(item)[0] === el);
    const cities = elObj.map(valueSelector);
    res.push([el, cities]);
  });
  return res;
}
